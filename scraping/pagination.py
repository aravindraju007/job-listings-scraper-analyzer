...
# Function to check if there is a next page of job listings
def has_next_page(soup):
    # Check for a next page by finding the 'Next' button
...
def main():
    ...
    total_pages = 10  # Maximum number of pages to scrape
    ...
    while page < total_pages:
        ...
        # Parse the page to check if there is a next page
        response = requests.get(f'{BASE_URL}&start={page * 10}')
        soup = BeautifulSoup(response.text, 'html.parser')
